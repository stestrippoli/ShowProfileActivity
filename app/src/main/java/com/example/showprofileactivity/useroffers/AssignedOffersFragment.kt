...
class AssignedOffersFragment : Fragment() {
    private var columnCount = 1
    private val offers = AssignedOffersCollection
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_assigned_offers, container, false)

        offers.clear()
        for (offer in vm.offers.value!!)
            if (offer.accepted == true && offer.acceptedUser == FirebaseAuth.getInstance().currentUser?.displayName && offer.completed == false)
        ...
        if (offersView is RecyclerView) {
            ...
            with(offersView) {
                ...
                adapter = MySavedOffersRecyclerViewAdapter(AssignedOffersCollection.ITEMS, object: MySavedOffersRecyclerViewAdapter.ItemClickListener {
